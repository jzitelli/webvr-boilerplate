...
THREE.VREffect = function ( renderer, onError ) {
	...
	var updateSeparationMatrices = function ( scale ) {
		...
		if ( vrHMD ) {
			...
			this.eyeTransL = eyeTransL.x;
			this.eyeTransR = eyeTransR.x;
		}
	...
	var updateProjectionMatrices = function ( near, far ) {
		...
		if ( vrHMD ) {
			...
			this.upDegrees    = eyeFOVR.upDegrees;
			this.downDegrees  = eyeFOVR.downDegrees;
			this.leftDegrees  = eyeFOVR.leftDegrees;
			this.rightDegrees = eyeFOVR.rightDegrees;
	...
	this.downDegrees = 15;
	this.leftDegrees = 20;
	this.rightDegrees = 20;
	
	this.manualUpdate = function () {

		stereoTransformL.makeTranslation( this.eyeTransL, 0, 0 );
		stereoTransformR.makeTranslation( this.eyeTransR, 0, 0 );

		var eyeFOV = {
			upDegrees: this.upDegrees,
			downDegrees: this.downDegrees,
			leftDegrees: this.leftDegrees,
			rightDegrees: this.rightDegrees
		};

		cameraL.projectionMatrix = fovToProjection( eyeFOV, true, _near, _far );
		cameraR.projectionMatrix = fovToProjection( eyeFOV, true, _near, _far );
